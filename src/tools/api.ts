...
export function registerApiTools(server: McpServer): void {
  ...
  server.tool(
    "get_available_apis",
    "获取当前用户有权限的接口文档列表。需要先登录（调用 login），返回的接口列表包含 url、method、参数说明，可用于 call_api 调用或指导编码",
    {},
    async () => {
      try {
  ...
  server.tool(
    "call_api",
    "代理调用内部业务接口，自动携带认证 Token。需要先登录（调用 login）。GET 请求参数拼接为查询字符串，POST/PUT/DELETE 参数作为 JSON body",
    {
