...
class AuthClient {
   ...
   * 发起登录，获取授权链接和轮询码
   */
  async login(): Promise<{ authUrl: string; pollCode: string }> {
    const result = await this.request<{ authUrl: string; pollCode: string }>(
      "/auth/login"
    );
    return result;
  }
   ...
   * 轮询授权状态
   */
  async poll(pollCode: string): Promise<{ status: string; token?: string }> {
    const result = await this.request<{ status: string; token?: string }>(
      `/auth/poll?code=${encodeURIComponent(pollCode)}`
    );
    return result;
  }
   ...
   */
  async pollForToken(pollCode: string): Promise<string> {
   ...
   * 获取当前用户有权限的接口列表
   */
  async getApis(token: string): Promise<any[]> {
    const result = await this.request<any[]>("/api/v1/apis", {
