...
export function registerAuthTools(server) {
    // login tool
    server.tool("login", "发起钉钉扫码登录，自动打开浏览器完成授权。使用流程：先调用 get_auth_status 检查状态，未登录再调用此工具", {}, async () => {
        try {
            // 1. 检查 Token 缓存是否有效
    ...
    server.tool("get_auth_status", "检查当前登录状态", {}, async () => {
        if (tokenManager.isTokenValid()) {
            return success("已登录");
        }
        return success("未登录");
    ...
    // logout tool
    server.tool("logout", "登出，清除本地缓存的 Token", {}, async () => {
